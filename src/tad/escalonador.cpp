...
Escalonador::Escalonador(RedeLogistica* rede, int cap, double lat, double interv, double custo, int total_pacotes) :
    m_tempo_atual(0.0),
...
void Escalonador::executar() {
    while (!m_fila_eventos.estaVazia() && m_pacotes_entregues < m_total_pacotes) {
        ...
        if (evento_atual.timestamp < m_tempo_atual) continue;
    ...
    if (evento.tipo == TipoEvento::CHEGADA_PACOTE) {
        ...
        if (armazem_chegada_id == pacote->getDestinoFinalID()) {
            ...
            if (armazem_obj && proximo_hop_id != -1) {
                // ✨ LOG "ARMAZENADO" GERADO PELO ESCALONADOR ✨
                std::stringstream msg;
        ...
        if (armazem_origem) {
            // ✨ Passa o tempo atual para o armazém gerar o log de "removido de" ✨
            ...
            for (int i = 0; i < pacotes_a_enviar.GetTamanho(); ++i) {
                ...
                // ✨ LOG "EM TRANSITO" GERADO PELO ESCALONADOR ✨
                std::stringstream msg_transito;
            ...
            for (int i = 0; i < pacotes_a_rearmazanar.GetTamanho(); ++i) {
                ...
                // ✨ LOG "REARMAZENADO" (usando a string "armazenado") GERADO PELO ESCALONADOR ✨
                std::stringstream msg;
                msg << "armazenado em " << formatarID(origem_id, 3) << " na secao " << formatarID(destino_id, 3);
